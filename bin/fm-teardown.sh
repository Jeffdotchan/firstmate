...
# worktree has no commits.
# Scout tasks use the shared unresolved-decision completion gate below.
# An unanswered captain wait is not landed work and must not be reaped.
...
if [ "$FORCE" != "--force" ] && [ "$KIND" != secondmate ]; then
  ...
  # Scout teardown has already run the shared completion owner above.
  # Its terminal-status exception and explicit inventory attestation are more
  # precise than re-folding the raw status stream here.
  if [ "$KIND" != scout ]; then
    _td_open=''
