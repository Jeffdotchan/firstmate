...
#   has-active <origin>                    -> RETAINED local verb (not upstream). Exit 0 while any
#                                             captain-held task belongs to <origin>, so
#                                             bin/fm-teardown.sh can refuse teardown on an
#                                             unanswered captain call. Matches both the legacy
#                                             <origin>-decision-* identities and the collapsed
#                                             plain <origin> task id.
...
# RETAINED local verb, deliberately not carried upstream: bin/fm-teardown.sh
# gates teardown on it so a worktree cannot be removed while the captain still
# owes an answer. Upstream's `verify` is a different, inventory-completeness
# gate and does not answer "is a hold open right now". Superset of the
# pre-collapse behavior: matches the legacy <origin>-decision-<key> identities
# AND the collapsed plain <origin> task id.
command_has_active() {  # <origin-id>
  local origin=${1:-} rows
  [ "$#" -eq 1 ] || { usage >&2; exit 2; }
  validate_slug origin-id "$origin"
  command -v tasks-axi >/dev/null 2>&1 || fail "tasks-axi is required"
  rows=$(cd "$FM_HOME" && tasks-axi list --state held --kind captain 2>/dev/null) \
    || fail "could not list captain-held work for $origin"
  case "$rows" in
    *"${origin}-decision-"*) return 0 ;;
  esac
  printf '%s\n' "$rows" | grep -qE "(^|[[:space:]])${origin}([[:space:]]|$)" && return 0
  return 1
}
