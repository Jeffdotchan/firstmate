...
Any later commit makes that attestation stale.
Republish the updated branch with `git push no-mistakes` so the pipeline regenerates the attestation for the new head before the check runs again.
